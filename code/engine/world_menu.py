...
class World_Menu:
    ...
    def __init__(self, World):
        ...
        self.max_menu_idle_time = (
            25  # how long a menu should be up before its closed/cleared
        )
    ...
    def debug_menu(self, key):
        if self.menu_state == "none":
            ...
            self.text_queue.append(
                "9 - toggle after action report (AAR) mode (auto screenshots)"
            )
            ...
            if key == "1":
                ...
                engine.log.add_data(
                    "info", "comms check sent on all tactical_ai channels", True
                )
                ...
                for tactical in self.world.tactical_ai.values():
                    ...
                    for squad in tactical.squads:
                        ...
                        print(
                            f"radio recieve queue length: {len(squad.radio_receive_queue)}"
                        )
                ...
                engine.log.add_data(
                    "info",
                    f"After Action Report mode : {self.world.aar_mode_enabled}",
                    True,
                )
        ...
        if self.menu_state == "spawn_vehicles":
            ...
            if key == "1":
                ...
                engine.world_builder.spawn_object(
                    self.world,
                    [
                        self.world.player.world_coords[0],
                        self.world.player.world_coords[1],
                    ],
                    "german_sd_kfz_222",
                    True,
        ...
        if self.menu_state == "spawn_weapons":
            ...
            self.text_queue.append("1 - kampfpistole ")
            self.text_queue.append("2 - Panzerschreck")
            self.text_queue.append("3 - Panzerfaust 60 ")
        ...
        if self.menu_state == "spawn_misc":
            ...
            self.text_queue.append("3 - pinus_sylvestris")
            self.text_queue.append("4 - Pickle Jar")
            self.text_queue.append("5 - vodka ")
            ...
            if key == "1":
                engine.world_builder.spawn_object(
                    self.world,
                    [
                        self.world.player.world_coords[0] + 40,
                        self.world.player.world_coords[1],
                    ],
                    "dani",
                    True,
                ...
                engine.world_builder.spawn_object(
                    self.world,
                    [
                        self.world.player.world_coords[0] + 60,
                        self.world.player.world_coords[1],
                    ],
                    "feldfunk_battery_charger",
                    True,
                )
            elif key == "3":
                engine.world_builder.spawn_object(
                ...
                rotation = 270
                coords = engine.math_2d.get_column_coords(
                    self.world.player.world_coords, 80, count, rotation, 2
                )
                for _ in range(count):
                    temp = engine.world_builder.spawn_object(
                        self.world, coords.pop(), "concrete_square", True
                    )
    ...
    def eat_drink_menu(self, key):
        ...
        self.text_queue.append(
            "Blood Pressure: " + str(round(self.selected_object.ai.blood_pressure, 1))
        )
        self.text_queue.append(
            "Hunger: " + str(round(self.selected_object.ai.hunger, 1))
        )
        self.text_queue.append(
            "Thirst: " + str(round(self.selected_object.ai.thirst, 1))
        )
        self.text_queue.append(
            "Fatigue " + str(round(self.selected_object.ai.fatigue, 1))
        )
    ...
    def engine_menu(self, key):
        if "task_vehicle_crew" in self.world.player.ai.memory:
            ...
            for b in vehicle.ai.engines:
                self.text_queue.append(
                    f"{b.name} {'[on]' if b.ai.engine_on else '[off]'}"
                )

            for b in vehicle.ai.batteries:
                self.text_queue.append(
                    f"{b.name} charge {round(b.ai.state_of_charge)}/{b.ai.max_capacity}"
                )
    ...
    def exit_world_menu(self, key):
        ...
        self.text_queue.append(
            "Current coordinates "
            + str(engine.math_2d.get_round_vector_2(self.world.player.world_coords))
        )
    ...
    def fuel_menu(self, key):
        ...
        self.text_queue.append(
            "Current Fuel Load : " + str(fuel_tank.ai.inventory[0].volume)
        )
    ...
    def generic_item_menu(self, key):
        ...
        if self.selected_object.name is None:
            engine.log.add_data(
                "error",
                "world_menu.generic_item_menu - selected obj name is none",
                True,
            )
            self.deactivate_menu()
            return
        ...
        if distance < self.max_menu_distance:
            if (
                self.selected_object.is_human == False
                and self.selected_object.volume < 21
                and self.selected_object.weight < 50
            ):
    ...
    def hit_marker_menu(self, key):
        ...
        self.text_queue.append(
            f"object: {self.selected_object.ai.hit_data.hit_object_name}"
        )
        self.text_queue.append(
            f"projectile: {self.selected_object.ai.hit_data.projectile_name}"
        )
        self.text_queue.append(
            f"penetration: {self.selected_object.ai.hit_data.penetrated}"
        )
        self.text_queue.append(f"hit side: {self.selected_object.ai.hit_data.hit_side}")
        self.text_queue.append(
            f"hit compartment: {self.selected_object.ai.hit_data.hit_compartment}"
        )
        self.text_queue.append(f"distance: {self.selected_object.ai.hit_data.distance}")
        self.text_queue.append(
            f"result/damage: {self.selected_object.ai.hit_data.result}"
        )
    ...
    def human_menu(self, key):
        ...
        if distance < 500:
            ...
            self.text_queue.append(
                "Blood Pressure: "
                + str(round(self.selected_object.ai.blood_pressure, 1))
            )
            self.text_queue.append(
                "Hunger: " + str(round(self.selected_object.ai.hunger, 1))
            )
            ...
            self.text_queue.append(
                "Confirmed Kills: " + str(self.selected_object.ai.confirmed_kills)
            )
            self.text_queue.append(
                "Probable Kills: " + str(self.selected_object.ai.probable_kills)
            )
            ...
            if self.selected_object.ai.wearable_head is not None:
                self.text_queue.append(
                    f"Head: {self.selected_object.ai.wearable_head.name}"
                )
            if self.selected_object.ai.wearable_upper_body is not None:
                self.text_queue.append(
                    f"Head: {self.selected_object.ai.wearable_upper_body.name}"
                )
            if self.selected_object.ai.primary_weapon != None:
                ammo_gun, ammo_inventory, magazine_count = (
                    self.selected_object.ai.check_ammo(
                        self.selected_object.ai.primary_weapon, self.selected_object
                    )
                )
                self.text_queue.append(
                    "[primary weapon]: " + self.selected_object.ai.primary_weapon.name
                )
                ...
                self.text_queue.append(
                    "- rounds Fired: "
                    + str(self.selected_object.ai.primary_weapon.ai.rounds_fired)
                )
                self.text_queue.append(
                    f"- accuracy: {self.selected_object.ai.primary_weapon.ai.get_accuracy()}"
                )
            if self.selected_object.ai.throwable != None:
                throwing_text = (
                    f"[throwing weapon]: {self.selected_object.ai.throwable.name}"
                )
            ...
            if self.selected_object.ai.antitank != None:
                self.text_queue.append(
                    "[anti-tank weapon]: " + self.selected_object.ai.antitank.name
                )
            ...
            for (
                currency_name,
                currency_amount,
            ) in self.selected_object.ai.wallet.items():
                self.text_queue.append(currency_name + ": " + str(currency_amount))

            self.text_queue.append("")

            self.text_queue.append(
                "Squad Size: " + str(len(self.selected_object.ai.squad.members))
            )
        ...
        if self.world.debug_mode == True:
            ...
            self.text_queue.append(
                "rotation angle: " + str(self.selected_object.rotation_angle)
            )
            d = engine.math_2d.get_distance(
                self.world.player.world_coords, self.selected_object.world_coords
            )
            ...
            if self.selected_object.ai.squad.squad_leader != None:
                d2 = engine.math_2d.get_distance(
                    self.selected_object.world_coords,
                    self.selected_object.ai.squad.squad_leader.world_coords,
                )
            self.text_queue.append(
                "current task: " + self.selected_object.ai.memory["current_task"]
            )
            if self.selected_object.ai.in_vehicle():
                self.text_queue.append(
                    "Vehicle Role: "
                    + self.selected_object.ai.memory["task_vehicle_crew"]["role"]
                )
            self.text_queue.append("Distance from player: " + str(d))
            self.text_queue.append("Distance from squad: " + str(d2))
            self.text_queue.append(
                "AI in building: " + str(self.selected_object.ai.in_building)
            )
        ...
        if self.menu_state == "player_menu":
            ...
            if self.selected_object.ai.large_pickup != None:
                self.text_queue.append(
                    "6 - Drop " + self.selected_object.ai.large_pickup.name
                )
                if key == "6":
                    self.selected_object.ai.drop_object(
                        self.selected_object.ai.large_pickup
                    )
                    self.deactivate_menu()
            if key == "1":
            ...
            if key == "2":
                ...
                self.change_menu("storage")
            if key == "3":
                self.selected_object.ai.handle_event(
                    "speak", ["ask to upgrade gear", None]
                )
            if key == "4" and self.world.player.ai.in_vehicle():
                ...
                self.selected_object.ai.handle_event(
                    "speak", ["task_enter_vehicle", vehicle]
                )
            ...
            if key == "2":
                self.selected_object.ai.handle_event(
                    "speak", ["ask to join squad", self.world.player.ai.squad]
                )
                self.deactivate_menu()
    ...
    def radio_menu(self, key):
        ...
        self.text_queue.append(
            "Frequency: " + str(self.selected_object.ai.current_frequency)
        )
        self.text_queue.append("Volume: " + str(self.selected_object.ai.volume))
        self.text_queue.append(
            "Transmission Power: " + str(self.selected_object.ai.transmission_power)
        )
        if self.selected_object.ai.battery != None:
            self.text_queue.append(
                "Battery State of Charge: "
                + str(round(self.selected_object.ai.battery.ai.state_of_charge, 2))
                + "/"
        ...
        if self.menu_state == "none":
            ...
            if key == "5":
                if (
                    self.selected_object.ai.volume
                    < self.selected_object.ai.volume_range[1]
                ):
            ...
            if key == "6":
                if (
                    self.selected_object.ai.volume
                    > self.selected_object.ai.volume_range[0]
                ):
                    self.selected_object.ai.volume -= 1
                    self.radio_menu(None)
                    return
    ...
    def storage_menu(self, key):
        ...
        if distance > self.max_menu_distance:
            self.text_queue = []
            self.text_queue.append(
                "-- Storage Menu: " + self.selected_object.name + " --"
            )
            return
        ...
        if self.menu_state == "selected_object":
            ...
            if key == "1":
                ...
                self.storage_menu_selection.world_coords = copy.copy(
                    self.selected_object.world_coords
                )
                engine.math_2d.randomize_position_and_rotation(
                    self.storage_menu_selection
                )
        ...
        if self.menu_state == "add_object":
            self.text_queue = []
            self.text_queue.append(
                "-- Select a object to add to : " + self.selected_object.name + " --"
            )
            self.text_queue.append("z - back")
            ...
            if self.selected_object.is_player == False:
                if self.world.player.ai.large_pickup != None:
                    self.text_queue.append(
                        "x - add " + self.world.player.ai.large_pickup.name
                    )
                    if key == "x":
    ...
    # --------------------------------------------------------------------------
    def display_recent_hits(self, vehicle_obj, text_index=-1):
        """Display most recent hits, prioritizing penetrated ones"""
        if len(vehicle_obj.ai.collision_log) == 0:
            return None

        penetrated_hits = [h for h in vehicle_obj.ai.collision_log if h.penetrated]
        non_penetrated_hits = [
            h for h in vehicle_obj.ai.collision_log if not h.penetrated
        ]

        recent_penetrated = penetrated_hits[-2:] if len(penetrated_hits) > 0 else []
        remaining_slots = 2 - len(recent_penetrated)
        recent_non_penetrated = (
            non_penetrated_hits[-remaining_slots:]
            if remaining_slots > 0 and len(non_penetrated_hits) > 0
            else []
        )

        all_hits = recent_penetrated + recent_non_penetrated

        if len(all_hits) == 0:
            return None

        self.text_queue.append("-- Recent Hits --")
        for hit in all_hits[::-1]:
            status = "penetrated" if hit.penetrated else "blocked"
            self.text_queue.append(f"{hit.projectile_name}: {status}")
        self.text_queue.append("")
    ...
    def vehicle_menu(self, key):
        ...
        if self.menu_state == "external":
            ...
            self.text_queue = []
            self.text_queue.append(
                "--External Vehicle Menu : " + self.selected_object.name + " --"
            )
            self.text_queue.append("Vehicle : " + self.selected_object.name)
            if self.selected_object.ai.vehicle_disabled:
                self.text_queue.append("Vehicle Disabled")
            self.text_queue.append("")
            self.display_recent_hits(self.selected_object)
            ...
            if distance < self.max_menu_distance:
                ...
                if distance < 300 and self.world.player.ai.in_vehicle():
                    ...
                    if key == "5":
                        self.world.text_queue.append(
                            "[You attach the vehicle for towing]"
                        )
                ...
                if key == "2":
                    # enter the vehicle
                    self.world.player.ai.switch_task_enter_vehicle(
                        self.selected_object, None
                    )
                ...
                self.text_queue.append("Distance too far for actions")
                self.text_queue.append("1 - view diagnostics ")
                if key == "1":
                    self.world.vehicle_diagnostics = True
                    self.world.vehicle_diagnostics_vehicle = self.selected_object
                    return
        ...
        if self.menu_state == "internal":
            if "task_vehicle_crew" in self.world.player.ai.memory:
                currentRole = self.world.player.ai.memory["task_vehicle_crew"][
                    "vehicle_role"
                ]
                if currentRole == None:
                    currentRole = "none"
                ...
                self.text_queue.append(
                    f"Your BP: {self.world.player.ai.blood_pressure}"
                )
                self.text_queue.append("")

                if radio:
                    self.text_queue.append(
                        "Radio : " + self.selected_object.ai.radio.name
                    )
                ...
                if self.selected_object.ai.towed_object != None:
                    ...
                    if key == "6":
                        self.world.text_queue.append(
                            "[You detach the vehicle from the tow bar]"
                        )
                ...
                if key == "4":
                    # flip the bool
                    self.world.display_vehicle_text = (
                        not self.world.display_vehicle_text
                    )
                    # refresh the text
                    self.vehicle_menu("none")
                if key == "5" and radio:
        ...
        if self.world.debug_mode == True:
            if self.selected_object != None:
                ...
                self.text_queue.append(
                    "throttle: " + str(self.selected_object.ai.throttle)
                )
                self.text_queue.append(
                    "brake power: " + str(self.selected_object.ai.brake_power)
                )
                self.text_queue.append(
                    "wheel steering: " + str(self.selected_object.ai.wheel_steering)
                )
                self.text_queue.append(
                    "vehicle speed: " + str(self.selected_object.ai.current_speed)
                )
                self.text_queue.append(
                    "acceleration: " + str(self.selected_object.ai.acceleration)
                )
                ...
                for b in self.selected_object.ai.turrets:
                    ...
                    if b.ai.primary_weapon != None:
                        ...
                        ammo_gun, ammo_inventory, magazine_count = (
                            self.world.player.ai.check_ammo(
                                b.ai.primary_weapon, self.selected_object
                            )
                        )
                        self.text_queue.append(
                            "-- "
                            + b.ai.primary_weapon.name
                            + ": "
                            + str(ammo_gun)
                            + "/"
                            + str(ammo_inventory)
                        ...
                        if b.ai.coaxial_weapon != None:
                            ammo_gun, ammo_inventory, magazine_count = (
                                self.world.player.ai.check_ammo(
                                    b.ai.coaxial_weapon, self.selected_object
                                )
                            )
                            self.text_queue.append(
                                "-- "
                                + b.ai.coaxial_weapon.name
                                + ": "
                                + str(ammo_gun)
                                + "/"
                                + str(ammo_inventory)
