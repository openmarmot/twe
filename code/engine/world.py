...
class World:
    ...
    def __init__(self):
        ...
        # spawn locations
        self.spawn_center = [0.0, 0.0]
        self.spawn_north = [0.0, -8000.0]
        self.spawn_south = [0.0, 8000.0]
        self.spawn_west = [-8000.0, 0.0]
        self.spawn_east = [8000.0, 0.0]
        self.spawn_far_east = [10000.0, 0.0]
        ...
        self.tactical_ai = {}
        self.tactical_ai["german"] = AIFactionTactical(
            self, "german", [], ["soviet"], 3
        )
        self.tactical_ai["soviet"] = AIFactionTactical(
            self, "soviet", [], ["german"], 5
        )
        ...
        self.friendly_fire_vehicle = (
            False  # False means friendly vehicles are added to the ignore list
        )
    ...
    def activate_context_menu(self):
        ...
        if self.world_menu.active_menu == "none":
            if self.player.ai.in_vehicle():
                vehicle = self.player.ai.memory["task_vehicle_crew"][
                    "vehicle_role"
                ].vehicle
    ...
    # ---------------------------------------------------------------------------
    def check_collision_return_object(
        self, collider, ignore_list, objects, consider_prone=False
    ):
        ...
        collided = engine.math_2d.checkCollisionCircleOneResult(
            collider, objects, ignore_list
        )
        if collided is not None:
            if collided.is_human:
                if collided.ai.in_vehicle():
                    collided = collided.ai.memory["task_vehicle_crew"][
                        "vehicle_role"
                    ].vehicle
                else:
    ...
    def check_map_bounds(self):
        ...
        for b in check_objects:
            if (
                b.world_coords[0] > self.map_size
                or b.world_coords[0] < -self.map_size
                or b.world_coords[1] > self.map_size
                or b.world_coords[1] < -self.map_size
            ):
    ...
    def check_object_exists(self, wo_obj):
        ...
        if wo_obj in self.add_queue:
            engine.log.add_data(
                "debug",
                f"check_object_exists on object: {wo_obj.name} in add_queue",
                True,
            )
            return False
        ...
        if wo_obj.grid_square:
            if wo_obj not in wo_obj.grid_square.wo_objects:
                engine.log.add_data(
                    "debug",
                    f"check_object_exists on object: {wo_obj.name} that is not in its grid square",
                    True,
                )
                return False
        else:
            engine.log.add_data(
                "debug",
                f"check_object_exists on object: {wo_obj.name} that does not have a grid square",
                True,
            )
            return False
        ...
        if wo_obj in self.remove_queue:
            engine.log.add_data(
                "debug",
                f"check_object_exists on object: {wo_obj.name} in remove_queue",
                True,
            )
            return False
            ...
            engine.log.add_data(
                "debug",
                f"check_object_exists on object: {wo_obj.name} in exit_queue",
                True,
            )
            return False
        else:
    ...
    def create_explosion(
        ...
        possible = self.grid_manager.get_objects_from_grid_squares_near_world_coords(
            world_coords, explosion_radius, True, True
        )
        ...
        # shrapnel
        if shrapnel_count > 0:
            engine.world_builder.spawn_shrapnel_cloud(
                self, world_coords, shrapnel_count, originator, weapon
            )

        # spawn effects
        engine.world_builder.spawn_object(self, world_coords, "dirt", True)
        engine.world_builder.spawn_explosion_and_fire(
            self, world_coords, fire_duration, smoke_duration
        )

    # ------------------------------------------------------------------------------
    def create_fire(
        self, world_coords, flame_amount, flame_radius, fire_duration, smoke_duration
    ):
        """create a fire that does damage"""
        possible = self.grid_manager.get_objects_from_grid_squares_near_world_coords(
            world_coords, flame_radius, True, True
        )
        for flame in range(flame_amount):
            coords = engine.math_2d.randomize_coordinates(
                world_coords, random.randint(1, 15)
            )
            engine.world_builder.spawn_explosion_and_fire(
                self, coords, fire_duration, smoke_duration
            )
            hit_list = engine.math_2d.checkCollisionCircleCoordsAllResults(
                coords, flame_radius, possible, []
            )
    ...
    def generate_ignore_list(self, obj):
        ...
        if obj is None:
            engine.log.add_data(
                "error", "world.generate_ignore_list input obj is None", True
            )
            return []
        ...
        if obj.is_human:
            ...
            if obj.ai.in_vehicle():
                ...
                ignore_list.append(
                    obj.ai.memory["task_vehicle_crew"]["vehicle_role"].vehicle
                )
                for b in obj.ai.memory["task_vehicle_crew"][
                    "vehicle_role"
                ].vehicle.ai.turrets:
            ...
            engine.log.add_data(
                "error", "world.generate_ignore_list obj is not human" + obj.name, True
            )
    ...
    def get_closest_vehicle(self, human, max_distance):
        ...
        possible_vehicles = (
            self.grid_manager.get_objects_from_grid_squares_near_world_coords(
                human.world_coords, max_distance, False, True
            )
        )
    ...
    def handle_keydown(self, key, mouse_screen_coords=None):
        ...
        if self.player.ai.in_vehicle():
            if self.player.ai.memory["task_vehicle_crew"][
                "vehicle_role"
            ].vehicle.is_airplane:
                if key == "p":
                    self.player.ai.switch_task_exit_vehicle(
                        self.player.ai.memory["task_vehicle_crew"][
                            "vehicle_role"
                        ].vehicle
                    )
            ...
            if self.player.ai.in_vehicle():
                ...
                if role.is_driver:
                    ...
                    if key == "w":
                        ...
                        self.player.ai.memory["task_vehicle_crew"][
                            "calculated_turret_angle"
                        ] = angle
                    if key == "s":
                        if turret.ai.coaxial_weapon:
                            # no calculated range for player
                            turret.ai.coaxial_weapon.ai.calculated_range = (
                                turret.ai.coaxial_weapon.ai.range
                            )
                    ...
                    if key == "a":
                        ...
                        # no calculated range for player
                        turret.ai.primary_weapon.ai.calculated_range = (
                            turret.ai.primary_weapon.ai.range
                        )
                ...
                if key == "w":
                    ...
                    if self.player.ai.primary_weapon != None:
                        self.player.ai.fire_player(
                            self.player.ai.primary_weapon, mouse_screen_coords
                        )
                    action = True

                if action:
                    self.player.ai.fatigue += (
                        self.player.ai.fatigue_add_rate * self.time_passed_seconds
                    )
            ...
            if b.is_vehicle or b.is_vehicle_wreck:
                for hit in b.ai.collision_log:
                    marker = engine.world_builder.spawn_object(
                        self, b.world_coords, "hit_marker", True
                    )
    ...
    def spawn_player(self):
        ...
        if len(candidates) > 0:
            ...
            engine.log.add_data(
                "error",
                "world.spawn_player but there are no humans left in the world of the correct faction: "
                + self.player_spawn_faction,
                True,
            )
            return False
    ...
    def update_debug_info(self):
        ...
        self.debug_text_queue.append(
            f"Grid Square Count: {len(self.grid_manager.index_map)}"
        )
        self.debug_text_queue.append(
            f"World Objects: {len(self.grid_manager.get_all_objects())}"
        )
        self.debug_text_queue.append(
            f"World Objects that Update: {len(self.grid_manager.get_all_wo_update())}"
        )
        self.debug_text_queue.append(
        ...
        self.debug_text_queue.append(
            f"Player memory current task: {self.player.ai.memory['current_task']}"
        )
        self.debug_text_queue.append(
            f"Player Scale Modifier: {self.player.scale_modifier}"
        )
        self.debug_text_queue.append(
            f"Player World Coords: {engine.math_2d.get_round_vector_2(self.player.world_coords)}"
        )
        self.debug_text_queue.append(
            f"Player Screen Coords: {self.player.screen_coords}"
        )
        self.debug_text_queue.append(f"Player altitude: {self.player.altitude}")
        self.debug_text_queue.append(
            f"Player Fatigue: {round(self.player.ai.fatigue, 1)}"
        )
        self.debug_text_queue.append(
            f"Player Speed: {self.player.ai.get_calculated_speed()}"
        )
        self.debug_text_queue.append(
            f"Player building overlap count: {len(self.player.ai.building_list)}"
        )
        self.debug_text_queue.append(
            f"Terrain at player coords: {self.player.grid_square.get_terrain_type(self.player.world_coords)}"
        )
        ...
        for b in self.tactical_ai.values():
            ...
            self.debug_text_queue.append(
                f"{b.faction} vehicles: {len(b.allied_crewed_vehicles)}"
            )
        ...
        if self.player.ai.in_vehicle():
            ...
            if vehicle_role.is_driver:
                ...
                self.vehicle_text_queue.append(
                    f"Electrical system on: {vehicle.ai.electrical_system_functioning}"
                )
                current_fuel, max_fuel = vehicle.ai.read_fuel_gauge()
                self.vehicle_text_queue.append(f"Fuel Gauge: {current_fuel}/{max_fuel}")
                self.vehicle_text_queue.append(
                    f"Speed: {round(vehicle.ai.current_speed, 1)}"
                )

                self.vehicle_text_queue.append(
                    f"max dynamic speed: {round(vehicle.ai.max_dynamic_speed, 1)} current speed: {round(vehicle.ai.current_speed, 1)}"
                )
                self.vehicle_text_queue.append(
                    f"acceleration: {vehicle.ai.acceleration}"
                )
                ...
                self.vehicle_text_queue.append(
                    f"wheel steering: {vehicle.ai.wheel_steering}"
                )

                # airplane specific
                if vehicle.is_airplane:
                    self.vehicle_text_queue.append(
                        f"altitude: {round(vehicle.altitude, 1)}"
                    )
                    self.vehicle_text_queue.append(
                        f"rate of climb: {round(vehicle.ai.rate_of_climb, 1)}"
                    )
                    self.vehicle_text_queue.append(
                        f"ailerons: {round(vehicle.ai.ailerons, 1)}"
                    )
                    self.vehicle_text_queue.append(
                        f"elevator: {round(vehicle.ai.elevator, 1)}"
                    )
                    self.vehicle_text_queue.append(
                        f"rudder: {round(vehicle.ai.rudder, 1)}"
                    )
                    self.vehicle_text_queue.append(
                        f"flaps: {round(vehicle.ai.flaps, 1)}"
                    )
            ...
            if vehicle_role.is_driver:
                for role in vehicle.ai.vehicle_crew:
                    if role.role_occupied:
                        if role.is_gunner:
                            self.vehicle_text_queue.append(
                                f"Gunner {role.human.name} Vehicle Targets: {len(role.human.ai.vehicle_targets)} Action: {role.human.ai.memory['task_vehicle_crew']['current_action']}"
                            )
            ...
            if vehicle_role.is_commander:
                for role in vehicle.ai.vehicle_crew:
                    if role.role_occupied:
                        ...
                        if role.is_driver:
                            self.vehicle_text_queue.append(
                                f"Driver {role.human.name}: {role.human.ai.memory['task_vehicle_crew']['current_action']}"
                            )

            if vehicle_role.is_gunner:
                turret = vehicle_role.turret
                self.vehicle_text_queue.append(
                    f"Vehicle angle: {vehicle.rotation_angle}"
                )
                ...
                if turret.ai.primary_weapon:
                    ...
                    ammo_gun, ammo_inventory, magazine_count = (
                        self.player.ai.check_ammo(weapon, vehicle)
                    )
                    self.vehicle_text_queue.append(
