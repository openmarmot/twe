...
class VehicleDiagnostics(object):
    ...
    def load_main_screen(self):
        ...
        for b in self.vehicle.ai.collision_log[-10:]:
            if b.penetrated:
                self.text_queue.append([f'penetrated: {b.penetrated} distance: {b.distance} projectile: {b.projectile_name} side: {b.hit_side} compartment: {b.hit_compartment} result: {b.result}',copy.copy(coord),self.text_red])
                coord[1]+=spacing
            else:
                self.text_queue.append([f'penetrated: {b.penetrated} distance: {b.distance} projectile: {b.projectile_name} side: {b.hit_side} compartment: {b.hit_compartment} result: {b.result}',copy.copy(coord),self.text_black])
                coord[1]+=spacing
