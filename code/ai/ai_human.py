...
class AIHuman:
    ...
    def building_check(self):
        ...
        for b in self.owner.grid_square.wo_objects_building:
            distance = engine.math_2d.get_distance(
                self.owner.world_coords, b.world_coords
            )
        ...
        if target.is_vehicle:
            ...
            penetration, pen_value, armor_value, _ = (
                engine.penetration_calculator.calculate_penetration(
                    ...
                    distance,
                    "steel",
                    target.ai.passenger_compartment_armor[hit_side],
                    hit_side,
                    relative_angle,
                )
            )
    ...
    def calculate_projectile_damage(self, projectile, distance):
            ...
            if self.wearable_head is None:
                ...
                penetration, pen_value, armor_value, _ = (
                    engine.penetration_calculator.calculate_penetration(
                        ...
                        distance,
                        "steel",
                        self.wearable_head.ai.armor[hit_side],
                        hit_side,
                        relative_angle,
                    )
                )
            ...
            if self.wearable_upper_body is None:
                ...
                penetration, pen_value, armor_value, _ = (
                    engine.penetration_calculator.calculate_penetration(
                        ...
                        distance,
                        "steel",
                        self.wearable_upper_body.ai.armor[hit_side],
                        hit_side,
                        relative_angle,
                    )
                )
        ...
        if (
            engine.penetration_calculator.projectile_data[
            ...
            if is_vehicle_crew:
                ...
                if self.memory["current_task"] != "task_exit_vehicle":
                    if self.primary_weapon is not None:
                        ...
                        engine.log.add_data(
                            "warn",
                            "close enemy and no primary weapon. not handled. faction:"
                            + self.squad.faction,
                            True,
                        )
    ...
    def event_collision(self, event_data):
        ...
        if event_data.is_projectile:
            distance = engine.math_2d.get_distance(
                self.owner.world_coords, event_data.ai.starting_coords
            )
            ...
            if event_data.ai.shooter is not None:
                ...
                engine.log.add_data(
                    "error", "projectile " + event_data.name + " shooter is none", True
                )

            # react
            if self.memory["current_task"] in [
                "task_vehicle_crew",
                "task_exit_vehicle",
            ]:
                # - we are in a vehicle -
            ...
            if random.randint(1, 5) == 1:
                ...
                if self.in_vehicle():
                    engine.log.add_data(
                        "warn", "hit by a grenade while in a vehicle", True
                    )
                else:
                    # - we are on foot -
                    if self.prone is True:
                        self.prone_state_change()
                    destination = [
                        self.owner.world_coords[0] + float(random.randint(-60, 60)),
                        self.owner.world_coords[1] + float(random.randint(-60, 60)),
                    ]
                    self.switch_task_move_to_location(destination, None)
            ...
            engine.log.add_data(
                "debug", "unidentified collision: " + event_data.name, True
            )
    ...
    def event_add_inventory(self, event_data):
        ...
        if event_data not in self.inventory:
                ...
                if event_data.is_gun:
                    if self.primary_weapon is not None:
                        if self.primary_weapon == event_data:
                            engine.log.add_data(
                                "error",
                                f"error in event_add_inventory {event_data.name} is already primary_weapon",
                            )
                        # drop the current obj and pick up the new one
                        self.drop_object(self.primary_weapon)
                    if self.owner.is_player:
                        self.owner.world.text_queue.insert(
                            0, "[ " + event_data.name + " equipped ]"
                        )
            ...
            engine.log.add_data(
                "error",
                "ai_human.event_add_inventory - obj already in inventory: "
                + event_data.name,
                True,
            )
    ...
    def event_explosion(self, event_data):
        ...
        if self.owner.is_player is False:
            if self.memory["current_task"] != "task_vehicle_crew":
                # move slightly
                coords = [
                    self.owner.world_coords[0] + random.randint(-15, 15),
                    self.owner.world_coords[1] + random.randint(-15, 15),
                ]
    ...
    def event_remove_inventory(self, event_data):
        ...
        if event_data in self.inventory:
            ...
            engine.log.add_data(
                "error",
                "ai_human.event_remove_inventory object is not in inventory: "
                + event_data.name,
                True,
            )
    ...
    def event_speak(self, event_data):
        ...
        if event_data[0] == "task_enter_vehicle":
            ...
            engine.log.add_data(
                "error", "speak inscruction: " + event_data[0] + " not handled", True
            )
        ...
        if weapon.ai.check_if_can_fire():
            ...
            if target.is_human:
                if target.ai.in_vehicle():
                    vehicle = target.ai.memory["task_vehicle_crew"][
                        "vehicle_role"
                    ].vehicle
                    ...
                    if target.ai.memory["current_task"] == "task_move_to_location":
                        destination = target.ai.memory["task_move_to_location"][
                            "destination"
                        ]
                        aim_coords = engine.math_2d.moveTowardsTarget(
                            target.ai.get_calculated_speed(),
                            aim_coords,
                            destination,
                            time_passed,
                        )
            ...
            aim_coords, calculated_range = self.calculate_human_accuracy(
                aim_coords, distance, weapon
            )
            weapon.rotation_angle = engine.math_2d.get_rotation(
                self.owner.world_coords, aim_coords
            )
    ...
    def fire_player(self, weapon, mouse_coords):
        ...
        if weapon.ai.check_if_can_fire():
            ...
            adjusted_coords, calculated_range = self.calculate_human_accuracy(
                mouse_coords, 0, weapon
            )
            rotation_angle = engine.math_2d.get_rotation(
                self.owner.screen_coords, adjusted_coords
            )
    ...
    def get_compatible_magazines_within_range(self, gun, max_distance):
        ...
        for b in self.owner.grid_square.wo_objects_gun_magazine:
            if gun.name in b.ai.compatible_guns:
                if (
                    engine.math_2d.get_distance(self.owner.world_coords, b.world_coords)
                    < max_distance
                ):
    ...
    def get_nearby_damaged_vehicles(self, max_range):
        ...
        for vehicle in self.owner.grid_square.wo_objects_vehicle:
            if vehicle.ai.vehicle_disabled:
                if (
                    engine.math_2d.get_distance(
                        self.owner.world_coords, vehicle.world_coords
                    )
                    < max_range
                ):
    ...
    def get_target(self, prefer_soft_target, prefer_vehicle):
        ...
        if prefer_soft_target:
            if self.human_targets and self.vehicle_targets:
                if (
                    self.vehicle_targets[0].ai.passenger_compartment_armor["right"][0]
                    < 2
                ):
    ...
    def give_squad_transportation_orders(self, vehicle_order):
        ...
        near_squad = self.owner.world.get_objects_within_range(
            self.owner.world_coords, self.squad.members, 800
        )
        ...
        possible_vehicles = self.owner.world.grid_manager.get_objects_from_grid_squares_near_world_coords(
            self.owner.world_coords, vehicle_distance, False, True
        )
        near_vehicle = self.owner.world.get_objects_within_range(
            self.owner.world_coords, possible_vehicles, vehicle_distance
        )
        ...
        for b in near_vehicle:
            ...
            # skip non-transport vehicles out of ammo
            if b.ai.vehicle_out_of_ammo and b.ai.is_transport is False:
                continue
            ...
            for c in self.owner.world.grid_manager.get_objects_from_all_grid_squares(
                True, False
            ):
    ...
    def handle_event(self, event, event_data):
        ...
        if event_data is None:
            engine.log.add_data(
                "error", f"ai_human.handle_event data for event {event} is None", True
            )
            return
        ...
        if event == "add_inventory":
            ...
            engine.log.add_data(
                "error", "ai_human.handle_event cannot handle event" + event, True
            )
    ...
    def handle_player_reload(self):
        ...
        if self.in_vehicle():
            ...
            if self.primary_weapon is not None:
                ammo_gun, ammo_inventory, magazine_count = self.check_ammo(
                    self.primary_weapon, self.owner
                )
                if ammo_inventory > 0:
    ...
    def launch_antitank(self, target_coords, mouse_screen_coords=None):
        ...
        if self.antitank is not None:
            if self.owner.is_player:
                ...
                adjusted_coords, calculated_range = self.calculate_human_accuracy(
                    mouse_screen_coords, 0, self.antitank
                )
                self.antitank.rotation_angle = engine.math_2d.get_rotation(
                    self.owner.screen_coords, adjusted_coords
                )
                ...
                distance = engine.math_2d.get_distance(
                    self.owner.world_coords, target_coords
                )
                adjusted_coords, calculated_range = self.calculate_human_accuracy(
                    target_coords, distance, self.antitank
                )
                self.antitank.rotation_angle = engine.math_2d.get_rotation(
    ...
    def pickup_object(self, world_object):
        ...
        if world_object.is_large_human_pickup:
            ...
            in_use = False
            for b in self.owner.world.grid_manager.get_objects_from_all_grid_squares(
                True, False
            ):
            ...
            if in_use:
                engine.log.add_data(
                    "warn", "Error large pick up is already picked up", True
                )
            else:
            ...
            if world_object.is_gun:
                if self.owner.is_player is False:
                    near_magazines = self.get_compatible_magazines_within_range(
                        world_object, 500
                    )
    ...
    def player_vehicle_role_change(self, requested_role):
        ...
        if self.in_vehicle():
            ...
            if requested_role in vehicle.ai.vehicle_crew:
                ...
                engine.log.add_data(
                    "warn",
                    "ai_human.player_vehicle_role_change - role not available in vehicle",
                    True,
                )
            ...
            engine.log.add_data(
                "warn",
                "ai_human.player_vehicle_role - current task is not task_vehicle_crew,"
                + str(self.memory["current_task"]),
                True,
            )
        ...
        if weapon.is_gun or weapon.is_handheld_antitank:
            ...
            if new_magazine is not None:
                if old_magazine is not None:
                    ...
                    if (
                        old_magazine.ai.disintegrating
                        and len(old_magazine.ai.projectiles) == 0
                    ):
                        # discard magazine
                        pass
                    else:
                ...
                if new_magazine in obj_with_inventory.ai.inventory:
                    ...
                    if new_magazine in obj_with_inventory.ai.ammo_rack:
                        ...
                        engine.log.add_data(
                            "error",
                            "ai_human.reload_weapon - new magazine not in inventory or ammo rack",
                            True,
                        )
                ...
                engine.log.add_data(
                    "error",
                    f"ai_human.reload_weapon {weapon.name}- no suitable magazines found",
                    True,
                )
                return False
        else:
            engine.log.add_data(
                "error", f"ai_human.reload_weapon {weapon.name}- not supported", True
            )
            return False
    ...
    def speak(self, what):
        ...
        if what != self.last_speak:
            ...
            distance = engine.math_2d.get_distance(
                self.owner.world_coords, self.owner.world.player.world_coords
            )
            if distance < 400:
                s = "[" + self.owner.name + "] "
        ...
        if order.order_defend_area:
            ...
            if distance > close_distance:
                ...
                if self.in_vehicle():
                    ...
                    if self.memory["task_vehicle_crew"][
                        "vehicle_role"
    ...
    def switch_task_engage_enemy(self, enemy):
        ...
        task_details = {"enemy": enemy, "last_think_time": 0, "think_interval": 0.5}
    ...
    def switch_task_medic(self, wounded_humans):
        ...
        if task_name in self.memory:
            ...
            task_details = {"wounded_humans": wounded_humans, "current_patient": None}
    ...
    def switch_task_think(self):
        ...
        if task_name in self.memory:
            ...
            task_details = {"something": "something"}
    ...
    def switch_task_vehicle_crew(self, vehicle, vehicle_order):
        ...
        vehicle_role = next(
            (role for role in vehicle.ai.vehicle_crew if not role.role_occupied), None
        )
        ...
        if vehicle_role is None:
            engine.log.add_data(
                "error",
                "ai_human.switch_task_vehicle_crew No role found!! Vehicle is full="
                + str(vehicle.ai.check_if_vehicle_is_full()),
                True,
            )
        ...
        task_details = {
            ...
            "current_action": VehicleCrewAction.NONE,
            "current_action_details": "",
            "target": None,
            "calculated_turret_angle": None,
            "engage_primary_weapon": False,
            "engage_coaxial_weapon": False,
            "engage_indirect_fire": False,
            "calculated_vehicle_angle": None,
            "calculated_distance_to_target": None,
            "last_think_time": 0,
            "think_interval": 0.5,
            "reload_start_time": 0,
            "vehicle_order": vehicle_order,
    ...
    def throw(self, target_coords, mouse_screen_coords=None):
        ...
        if self.throwable is not None:
            ...
            if self.owner.is_player:
                # do computations based off of where the mouse is. TARGET_COORDS is ignored
                self.throwable.rotation_angle = engine.math_2d.get_rotation(
                    self.owner.screen_coords, mouse_screen_coords
                )
                self.throwable.heading = engine.math_2d.get_heading_vector(
                    self.owner.screen_coords, mouse_screen_coords
                )
            else:
                self.throwable.rotation_angle = engine.math_2d.get_rotation(
    ...
    def transfer_liquid(self, from_object, to_object):
        ...
        if from_object.is_liquid and to_object.is_container:
            ...
            source_result, destination_result = engine.math_2d.get_transfer_results(
                source_amount, destination_amount, destination_maximum
            )
            from_object.volume = source_result

            # spawn a new liquid object with the amount to transfer
            z = engine.world_builder.spawn_object(
                self.owner.world, [0, 0], from_object.name, False
            )
            ...
            engine.log.add_data(
                "error", "ai_human.transfer_liquid - src/dest not recognized", True
            )
    ...
    def update(self):
        ...
        if self.blood_pressure > self.blood_pressure_min:
            ...
            if (
                self.owner.world.world_seconds - self.last_target_eval_time
                > self.target_eval_rate
            ) and self.owner.is_player is False:
            ...
            if (
                self.owner.world.world_seconds - self.last_building_check_time
                > self.building_check_rate
            ):
    ...
    def update_health(self):
        ...
        if self.blood_pressure > 0:
            if self.bleeding:
                if (
                    self.owner.world.world_seconds - self.last_bleed_time
                    > self.bleed_interval
                ):
                    self.last_bleed_time = (
                        self.owner.world.world_seconds + random.uniform(0, 2)
                    )
                    engine.world_builder.spawn_object(
                        self.owner.world, self.owner.world_coords, "small_blood", True
                    )
                ...
                self.morale = max(
                    0,
                    min(100, self.morale + 0.5 * self.owner.world.time_passed_seconds),
                )

            # -- body attribute stuff --
            if self.fatigue > 0:
                self.fatigue -= (
                    self.fatigue_remove_rate * self.owner.world.time_passed_seconds
                )
            ...
            # spawn body
            engine.world_builder.spawn_container_body(
                "body: " + self.owner.name, self.owner, 2
            )
    ...
    def update_large_pickup_position(self):
        ...
        self.large_pickup.rotation_angle = engine.math_2d.get_normalized_angle(
        ...
        self.large_pickup.world_coords = engine.math_2d.calculate_relative_position(
            ...
            self.large_pickup.large_pickup_offset,
        )
    ...
    def update_task_engage_enemy(self):
        ...
        if self.owner.world.world_seconds - last_think_time > think_interval:
            ...
            # reset time
            self.memory["task_engage_enemy"]["last_think_time"] = (
                self.owner.world.world_seconds
            )
            self.memory["task_engage_enemy"]["think_interval"] = random.uniform(
                0.1, 1.5
            )
            ...
            if enemy.is_human:
                ...
                engine.log.add_data(
                    "error",
                    f"ai_human.update_task_engage_enemy unkown object {enemy.name}",
                    True,
                )
    ...
    def update_task_engage_enemy_human(self):
        ...
        if distance < self.primary_weapon.ai.range:
            ...
            if new_enemy is None:
                ...
                if distance > (
                    self.primary_weapon.ai.range + self.primary_weapon.ai.range * 0.5
                ):
                    self.memory.pop("task_engage_enemy", None)
                    self.switch_task_think()
                else:
                    # move about 150 units closer to the enemy and re-engage
                    self.switch_task_move_to_location(
                        engine.math_2d.moveTowardsTarget(
                            150, self.owner.world_coords, enemy.world_coords, 1
                        ),
                        None,
                    )
            else:
    ...
    def update_task_engage_enemy_vehicle(self):
        ...
        if self.antitank is not None:
            ...
            engage, engage_reason = self.calculate_engagement(
                self.primary_weapon, enemy
            )
    ...
    def update_task_enter_vehicle(self):
        ...
        if (
            distance
            > self.memory["task_enter_vehicle"]["original_distance_to_vehicle"] + 100
        ):
    ...
    def update_task_exit_vehicle(self):
        ...
        if vehicle_role.human == self.owner:
            ...
            engine.log.add_data(
                "error",
                f"update_task_exit_vehicle vehicle_role.human does not match",
                True,
            )
        ...
        self.owner.reset_image = (
            True  # needed for dead guys i think because they don't move
        )
        ...
        if self.blood_pressure > 0:
            ...
            self.owner.world_coords = [
                self.owner.world_coords[0] + random.randint(5, 15),
                self.owner.world_coords[1] + random.randint(5, 15),
            ]
    ...
    def update_task_loot_container(self):
        ...
        if self.owner.world.check_object_exists(container) is False:
            ...
            distance = engine.math_2d.get_distance(
                self.owner.world_coords, container.world_coords
            )
    ...
    def update_task_mechanic(self):
        ...
        if self.memory["task_mechanic"]["current_vehicle"] is None:
            if len(self.memory["task_mechanic"]["damaged_vehicles"]) == 0:
                ...
                return
            self.memory["task_mechanic"]["current_vehicle"] = self.memory[
                "task_mechanic"
            ]["damaged_vehicles"].pop()
        current_vehicle = self.memory["task_mechanic"]["current_vehicle"]
        distance = engine.math_2d.get_distance(
            self.owner.world_coords, current_vehicle.world_coords
        )
        if distance > 1000:
        ...
        if distance > self.max_distance_to_interact_with_object:
            self.switch_task_move_to_location(
                current_vehicle.world_coords, current_vehicle
            )
            return
    ...
    def update_task_medic(self):
        ...
        if self.memory["task_medic"]["current_patient"] is None:
            if len(self.memory["task_medic"]["wounded_humans"]) == 0:
                ...
                return
            self.memory["task_medic"]["current_patient"] = self.memory["task_medic"][
                "wounded_humans"
            ].pop()
        ...
        distance = engine.math_2d.get_distance(
            self.owner.world_coords, patient.world_coords
        )
        if distance > 1000:
            self.memory["task_medic"]["current_patient"] = None
            return
    ...
    def update_task_move_to_location(self):
        ...
        if self.owner.world.world_seconds - last_think_time > think_interval:
            # reset time
            self.memory["task_move_to_location"]["last_think_time"] = (
                self.owner.world.world_seconds
            )
            ...
            if self.memory["task_move_to_location"]["moving_object"] is not None:
                ...
                if self.memory["task_move_to_location"]["moving_object"].is_vehicle:
                    if (
                        self.memory["task_move_to_location"][
                            "moving_object"
                        ].ai.check_if_vehicle_is_full()
                        is True
                    ):
                ...
                self.memory["task_move_to_location"]["destination"] = copy.copy(
                    self.memory["task_move_to_location"]["moving_object"].world_coords
                )
                # reset bot facing angle
                self.owner.rotation_angle = engine.math_2d.get_rotation(
                    self.owner.world_coords,
                    self.memory["task_move_to_location"]["destination"],
                )
                self.owner.reset_image = True
            ...
            if distance > self.max_walk_distance:
                b = self.owner.world.get_closest_vehicle(
                    self.owner, (self.max_walk_distance * 0.6)
                )
                if b is not None:
    ...
    def update_task_reload(self):
        ...
        if (
            self.owner.world.world_seconds
            - self.memory["task_reload"]["reload_start_time"]
    ...
    def update_task_squad_leader(self):
        ...
        if self.owner.world.world_seconds - last_think_time > think_interval:
            # reset time
            self.memory["task_squad_leader"]["last_think_time"] = (
                self.owner.world.world_seconds
            )
            ...
            if not has_defend_order:
                ...
                if decision == 1:
                    coords = [
                        self.owner.world_coords[0] + random.randint(-25, 25),
                        self.owner.world_coords[1] + random.randint(-25, 25),
                    ]
                    self.switch_task_move_to_location(coords, None)
    ...
    def update_task_sit_down(self):
        ...
        if self.memory["task_sit_down"]["status"] == "searching":
            ...
            if self.squad.faction in ["german", "soviet", "american"]:
                distance = 500
            temp = self.owner.world.get_closest_object(
                self.owner.world_coords,
                self.owner.grid_square.wo_objects_furniture,
                distance,
            )
        ...
        if self.memory["task_sit_down"]["status"] == "moving_to_object":
            distance = engine.math_2d.get_distance(
                self.owner.world_coords,
                self.memory["task_sit_down"]["furniture_object"].world_coords,
            )
            if distance < self.max_distance_to_interact_with_object:
                # sit down
                self.memory["task_sit_down"]["status"] = "sitting"
                self.memory["task_sit_down"]["sit_start_time"] = (
                    self.owner.world.world_seconds
                )
            else:
                self.switch_task_move_to_location(
                    self.memory["task_sit_down"]["furniture_object"].world_coords, None
                )
        ...
        if self.memory["task_sit_down"]["status"] == "sitting":
            ...
            if (
                self.owner.world.world_seconds
                - self.memory["task_sit_down"]["sit_start_time"]
                > self.memory["task_sit_down"]["sit_duration"]
            ):
                self.memory.pop("task_sit_down", None)
    ...
    def update_task_think(self):
        ...
        if self.primary_weapon and has_target:
            ...
            if closest:
                dist = engine.math_2d.get_distance(
                    self.owner.world_coords, closest.world_coords
                )
        ...
        if self.primary_weapon is None:
            ...
            if distance < 4000:
                gun = self.owner.world.get_closest_object(
                    self.owner.world_coords,
                    self.owner.grid_square.wo_objects_gun,
                    distance,
                )
            ...
            if self.check_ammo_bool(self.primary_weapon, self.owner):
                ...
                containers = self.owner.world.get_objects_within_range(
                    self.owner.world_coords,
                    self.owner.grid_square.wo_objects_container,
                    500,
                )
                if len(containers) > 0:
        ...
        if self.is_medic:
            wounded_humans = self.get_nearby_wounded_humans(
                self.squad.faction_tactical.allied_humans, 1000
            )
        ...
        distance = engine.math_2d.get_distance(
            self.owner.world_coords, self.squad.squad_leader.world_coords
        )
        # are we close enough to squad?
        if distance > self.squad_max_distance:
            self.switch_task_move_to_location(
                self.squad.squad_leader.world_coords, None
            )
            return
        else:
            ...
            if self.squad.squad_leader.ai.in_vehicle():
                squad_leader_vehicle = self.squad.squad_leader.ai.memory[
                    "task_vehicle_crew"
                ]["vehicle_role"].vehicle
                if (
                    squad_leader_vehicle.ai.requires_afv_training
                    and self.is_afv_trained
                ) or squad_leader_vehicle.ai.requires_afv_training is False:
    ...
    def update_task_think_idle(self):
        ...
        if decision == 1:
            # go for a walk
            coords = [
                self.owner.world_coords[0] + random.randint(-45, 45),
                self.owner.world_coords[1] + random.randint(-45, 45),
            ]
            self.switch_task_move_to_location(coords, None)
        elif decision == 2:
            # check containers
            ...
            if self.closest_building is not None:
                self.switch_task_move_to_location(
                    self.closest_building.world_coords, None
                )
