...
class AIFactionTactical:
    def __init__(
        self, world, faction, allied_factions, hostile_factions, radio_frequency
    ):
        ...
        self.radio = engine.world_builder.spawn_object(
            world, [0, 0], "radio_feldfu_b", False
        )
    ...
    def get_area_enemy_count(self, area):
        ...
        if self.faction == "civilian":
            ...
            print(
                "debug: ai_faction_tactical.get_area_enemy_count - faction not handled: ",
                self.faction,
            )
            return 0
    ...
    def get_area_friendly_count(self, area):
        ...
        if self.faction == "german":
            ...
            print(
                "debug: ai_faction_tactical.get_area_friendly_count - faction not handled: ",
                self.faction,
            )
    ...
    def set_initial_orders_and_positions(self):
        ...
        towed_gun_squads = []
        non_towed_motorized_squads = []
        for s in motorized_squads:
            if any(vehicle.ai.is_towed_gun for vehicle in s.vehicles):
                towed_gun_squads.append(s)
            else:
                non_towed_motorized_squads.append(s)

        motorized_squads = non_towed_motorized_squads[:]
        random.shuffle(towed_gun_squads)
        ...
        if self.initial_controlled_world_areas:
            ...
            for area in areas:
                # Assign 1 towed gun squad if available
                if towed_gun_squads:
                    squad = towed_gun_squads.pop(0)
                ...
                # Assign up to 2 foot squads
                count = 0
                while foot_squads and count < 2:
        ...
        remaining_squads = towed_gun_squads + motorized_squads + foot_squads
        if remaining_squads:
            ...
            for area in areas:
                for squad_list in [motorized_squads, foot_squads]:
                    if len(area_assignments[area]) >= min_squads_per_area:
                        break
                    while (
                        squad_list and len(area_assignments[area]) < min_squads_per_area
                    ):
            ...
            if overflow:
                ...
                while overflow:
                    ...
                    if len(area_assignments[area]) < max_squads_per_area:
                        ...
                        len(area_assignments[a]) >= max_squads_per_area for a in areas
                    ):
    ...
    # ---------------------------------------------------------------------------
    def set_squad_defend_order(
        self, squad, world_area, world_area_location, threat_direction=None
    ):
    ...
    def set_squad_starting_position(self, squad, position, threat_direction=None):
        ...
        for vehicle in squad.vehicles:
            ...
            while len(member_vehicle_assignments) > 0 and crew_count > 0:
                crew_count -= 1
                member_vehicle_assignments.pop().ai.switch_task_enter_vehicle(
    ...
    def split_squad(self, members):
        ...
        if len(members) > 0:
            ...
            squad.name = (
                f"squad_split_{squad_number}"  # for now just to keep track of these
            )
            ...
            for member in squad.members:
                if member.ai.large_pickup is not None:
                    if member.ai.large_pickup.is_radio:
                        member.ai.large_pickup.ai.current_frequency = (
    ...
    def update_human_lists(self):
        ...
        for b in self.world.grid_manager.get_objects_from_all_grid_squares(True, False):
            if b.ai.squad != None:
                if (
                    b.ai.squad.faction == self.faction
                    or b.ai.squad.faction in self.allied_factions
                ):
                    ...
                    if "task_vehicle_crew" in b.ai.memory:
                        if (
                            b.ai.memory["task_vehicle_crew"]["vehicle_role"].vehicle
                            not in self.allied_crewed_vehicles
                        ):
                            self.allied_crewed_vehicles.append(
                                b.ai.memory["task_vehicle_crew"]["vehicle_role"].vehicle
                            )
                else:
