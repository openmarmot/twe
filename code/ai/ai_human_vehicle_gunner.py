...
class AIHumanVehicleGunner:
    ...
    def action(self):
        ...
        if (
            self.owner.ai.memory["task_vehicle_crew"]["target"] is None
            and self.owner.ai.memory["task_vehicle_crew"]["engage_indirect_fire"]
            and self.owner.ai.memory["task_vehicle_crew"]["fire_missions"]
        ):
    ...
    def action_align_turret_forward(self):
        ...
        current_turret_angle = engine.math_2d.get_normalized_angle(
            turret.rotation_angle
        )
        angle_diff = abs((vehicle_angle - current_turret_angle + 180) % 360 - 180)

        if angle_diff > 5:
            self.owner.ai.memory["task_vehicle_crew"]["calculated_turret_angle"] = (
                vehicle.rotation_angle
            )
    ...
    def action_engage_indirect_fire(self):
        ...
        if self.rotate_turret(
            turret, self.owner.ai.memory["task_vehicle_crew"]["calculated_turret_angle"]
        ):
    ...
    def action_engage_target(self):
        ...
        if self.rotate_turret(
            ...
            if target.is_vehicle:
                if (
                    target.ai.check_if_vehicle_is_occupied() is False
                    or target.ai.vehicle_disabled
                ):
                    self.owner.ai.memory["task_vehicle_crew"]["target"] = None
                    self.owner.ai.memory["task_vehicle_crew"]["current_action"] = (
                        VehicleCrewAction.SCANNING
    ...
    def calculate_turret_aim(self, turret, target, weapon):
        ...
        weapon.ai.calculated_range = distance + random.randint(
            0, 500
        )  # your existing over-aim
        ...
        angular_dispersion = (
            weapon.ai.mechanical_accuracy_deg
        )  # e.g. base 0.2-0.5 deg for good guns
    ...
    def calculate_turret_aim_old(self, turret, target, weapon):
        ...
        # final results
        aim_coords = [
            aim_coords[0] + random.uniform(-adjust_max, adjust_max),
            aim_coords[1] + random.uniform(-adjust_max, adjust_max),
        ]
        ...
        self.owner.ai.memory["task_vehicle_crew"]["calculated_turret_angle"] = (
            engine.math_2d.get_rotation(turret.world_coords, aim_coords)
        )
    ...
    def check_vehicle_turret_rotation_real_angle(self, rotation_angle, turret):
        ...
        vehicle_angle = engine.math_2d.get_normalized_angle(
            turret.ai.vehicle.rotation_angle
        )
        target_angle = engine.math_2d.get_normalized_angle(rotation_angle)
        required_delta = (target_angle - vehicle_angle + 180) % 360 - 180
        return (
            turret.ai.rotation_range[0] <= required_delta <= turret.ai.rotation_range[1]
        )
    ...
    def think(self):
        ...
        if turret is None:
            print(
                f"turret none in {vehicle.name}, role {self.owner.ai.memory['task_vehicle_crew']['vehicle_role'].role_name}"
            )
            for b in vehicle.ai.vehicle_crew:
                print(
                    f" role: {b.role_name} is_driver: {b.is_driver} is_gunner: {b.is_gunner} is_commander {b.is_commander}"
                )
        ...
        # check main gun ammo
        ammo_gun, ammo_inventory, magazine_count = self.owner.ai.check_ammo(
        ...
        # special case
        if out_of_ammo_primary and turret.ai.primary_turret:
            vehicle.ai.vehicle_out_of_ammo = True

        if out_of_ammo_coax and out_of_ammo_primary:
            self.owner.ai.memory["task_vehicle_crew"]["current_action"] = (
                VehicleCrewAction.OUT_OF_AMMO
            )
            self.owner.ai.memory["task_vehicle_crew"]["target"] = None
            return
        ...
        if weapons_damaged:
            ...
            self.owner.ai.memory["task_vehicle_crew"]["current_action"] = (
                VehicleCrewAction.WEAPONS_DAMAGED
            )
            self.owner.ai.memory["task_vehicle_crew"]["target"] = None
            return
        ...
        if self.owner.ai.memory["task_vehicle_crew"]["target"] is not None:
            ...
            if (
                self.owner.ai.memory["task_vehicle_crew"]["fire_missions"]
                and out_of_ammo_primary == False
            ):
    ...
    def think_examine_target(self, out_of_ammo_primary, out_of_ammo_coax):
        ...
        rotation_check = self.check_vehicle_turret_rotation_real_angle(
            rotation_angle, turret
        )
        ...
        if out_of_ammo_primary is False:
            engage_primary, engage_primary_reason = self.owner.ai.calculate_engagement(
                turret.ai.primary_weapon, target
            )
            ...
            if engage_primary is False and target.is_vehicle:
                if engage_primary_reason == "":
                    ...
                    if turret.ai.primary_weapon.ai.magazine:
                        if (
                            turret.ai.primary_weapon.ai.magazine.ai.use_antitank
                            is False
                        ):
        ...
        if out_of_ammo_coax is False:
            engage_coaxial, engage_coaxial_reason = self.owner.ai.calculate_engagement(
                turret.ai.coaxial_weapon, target
            )
        ...
        if rotation_check:
            if engage_primary:
                if engage_coaxial:
                    ...
                    if target.is_human:
                        ...
                        if turret.ai.primary_weapon.ai.magazine:
                            if (
                                turret.ai.primary_weapon.ai.magazine.ai.use_antipersonnel
                                == False
                            ):
                                engage_primary = False

            # save the engagement commands for the gunner actions
            self.owner.ai.memory["task_vehicle_crew"]["engage_primary_weapon"] = (
                engage_primary
            )
            self.owner.ai.memory["task_vehicle_crew"]["engage_coaxial_weapon"] = (
                engage_coaxial
            )
            ...
            if engage_primary:
                ...
                self.owner.ai.memory["task_vehicle_crew"]["current_action_details"] = (
                    f"Engaging {target_name}"
                )
                return
            if engage_coaxial:
    ...
    def think_fire_mission(self):
        ...
        distance = engine.math_2d.get_distance(
            self.owner.world_coords, fire_mission.world_coords
        )
        ...
        # check rotation
        rotation_angle = engine.math_2d.get_rotation(
            turret.world_coords, fire_mission.world_coords
        )
        rotation_check = self.check_vehicle_turret_rotation_real_angle(
            rotation_angle, turret
        )
                ...
                if weapon.world_builder_identity in m.ai.compatible_guns:
                    if len(m.ai.projectiles) > 0:
                        if m.ai.use_antitank and m.ai.use_antipersonnel:
                            ...
                            engine.log.add_data(
                                "error",
                                f"ai_human.think_reload magazine {m.name} unknown use",
                                True,
                            )
