...
class AIHumanVehicle:
            ...
            if role.is_gunner:
                if important_hit.projectile_shooter:
                    if important_hit.projectile_shooter.is_turret:
                        self.owner.ai.memory["task_vehicle_crew"]["target"] = (
                            important_hit.projectile_shooter.ai.vehicle
                        )
                    else:
                        self.owner.ai.memory["task_vehicle_crew"]["target"] = (
                            important_hit.projectile_shooter
                        )
                    return
    ...
    def update_task_vehicle_crew(self):
        ...
        if self.owner.world.world_seconds - last_think_time > think_interval:
            # reset time
            self.owner.ai.memory["task_vehicle_crew"]["last_think_time"] = (
            ...
            # set think intervals - faster rates take precedence when roles overlap
            if role.is_driver:
            ...
            if role.is_commander:
                ...
                if self.owner.ai.in_vehicle():
                    ...
                    if role.is_gunner:
                        ...
                        is_gunner_busy = current_action in (
